...
describe.skipIf(!hasDatabaseUrl)("Phase 13 AP2: Campaign-Management-Service", () => {
  ...
  async function createActiveQuestionnaire(tenantId: string, key: string) {
    ...
    const question = await rawClient.question.create({
      data: { tenantId, questionnaireVersionId: questionnaireVersion.id, key: "q-choice", sortOrder: 1 },
    });
    ...
    await rawClient.answerOption.createMany({
      data: [
        { tenantId, questionVersionId: questionVersion.id, key: "BASIC", label: "Basic", sortOrder: 1 },
        { tenantId, questionVersionId: questionVersion.id, key: "PREMIUM", label: "Premium", sortOrder: 2 },
      ],
    });
  ...
  it("createCampaign() mit bereits vergebenem key -> CampaignKeyAlreadyExistsError", async () => {
    ...
    await expect(
      runWithTenantContext(ctx(tenantId, userId), () => createCampaign({ key: "dup", name: "Zweite" })),
  ...
  it("derselbe key ist in ZWEI verschiedenen Mandanten unabhaengig zulaessig (Tenant-Isolation)", async () => {
    ...
    await expect(
      runWithTenantContext(ctx(tenantA, userA), () => createCampaign({ key: "shared-key", name: "A" })),
    ).resolves.toBeDefined();
    await expect(
      runWithTenantContext(ctx(tenantB, userB), () => createCampaign({ key: "shared-key", name: "B" })),
    ).resolves.toBeDefined();
  });
