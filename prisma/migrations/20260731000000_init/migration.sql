...
CREATE TYPE "AnswerType" AS ENUM ('SINGLE_CHOICE', 'MULTIPLE_CHOICE', 'BOOLEAN', 'INTEGER', 'DECIMAL', 'SHORT_TEXT', 'DATE');
...
CREATE TYPE "VisibilityOperator" AS ENUM ('EQUALS', 'NOT_EQUALS', 'GREATER_THAN', 'GREATER_THAN_OR_EQUAL', 'LESS_THAN', 'LESS_THAN_OR_EQUAL', 'IN', 'NOT_IN', 'CONTAINS', 'IS_ANSWERED', 'IS_NOT_ANSWERED');
CREATE TYPE "LogicalCombinator" AS ENUM ('AND', 'OR');
...
CREATE TYPE "AnalyticsEventType" AS ENUM ('CONSULTATION_STARTED', 'CONSULTATION_TOPIC_OPENED', 'QUESTIONNAIRE_STARTED', 'QUESTION_ANSWERED', 'ANSWER_CHANGED', 'PATH_RECALCULATED', 'QUESTIONNAIRE_COMPLETED', 'NEED_DETECTED', 'OPPORTUNITY_OFFERED', 'OPPORTUNITY_DECLINED', 'RECOMMENDATION_GENERATED', 'RECOMMENDATION_ACCEPTED', 'RECOMMENDATION_REJECTED', 'CONSULTATION_COMPLETED', 'CONSULTATION_ABANDONED', 'DEAL_CLOSED', 'FOLLOW_UP_CREATED');
...
CREATE TABLE "visibility_conditions" (
  ...
  "combinator" "LogicalCombinator" NOT NULL DEFAULT 'AND',
  "created_at" TIMESTAMPTZ NOT NULL DEFAULT now(),
  PRIMARY KEY ("id")
);
...
ALTER TABLE "consultation_sessions" ADD CONSTRAINT "consultation_sessions_tenant_id_questionnaire_version_id_fkey" FOREIGN KEY ("tenant_id", "questionnaire_version_id") REFERENCES "questionnaire_versions" ("tenant_id", "id") ON DELETE RESTRICT;
...
-- =============================================================================
-- Phase 3A: Ergaenzungen fuer die Fragen-Engine (Frage-Versionierung,
-- Sichtbarkeitsbedingungen, Antwort-Historie). Wie bei Phase 2B handelt es
-- sich um von Hand gepflegtes SQL, das der Transpiler (scripts/schema_to_sql.py)
-- nicht erzeugen kann, und das bei jeder Neugenerierung der Datei manuell
-- wieder angehaengt wird (siehe docs/DECISION_LOG.md).
-- =============================================================================

-- -----------------------------------------------------------------------
-- 4) Exclusion Constraint gegen ueberlappende QuestionVersion-Gueltigkeitszeitraeume
--    (identisches Muster zu Abschnitt 2 / questionnaire_versions_no_overlap)
-- -----------------------------------------------------------------------

ALTER TABLE "question_versions" ADD CONSTRAINT "question_versions_no_overlap"
  EXCLUDE USING gist (
    "tenant_id" WITH =, "question_id" WITH =, tstzrange("valid_from", "valid_to", '[)') WITH &&
  ) WHERE ("status" IN ('ACTIVE', 'EXPIRED'));

-- -----------------------------------------------------------------------
-- 5) ConsultationSession.questionnaire_version_id: echte Unveraenderlichkeit
--    nach dem Setzen (nicht nur Konvention auf Service-Ebene). Sobald eine
--    Beratungssitzung mit einer Fragebogen-Version gestartet wurde, darf sich
--    diese Zuordnung nie mehr aendern, auch nicht rueckwirkend durch spaetere
--    Fragebogen-Aenderungen.
--
--    Hinweis zum Migrationsmodell: Dieses Projekt generiert pro Phase eine
--    einzige, frische init-Migration (kein `prisma migrate dev`-Verlauf mit
--    inkrementellen ALTER-Schritten gegen eine befuellte Produktions-DB,
--    siehe docs/DECISION_LOG.md). Die Verifikations-DB ist bei jedem Lauf
--    leer, daher ist die Spalte hier direkt als NOT NULL angelegt und ein
--    klassisches Backfill-UPDATE waere auf dieser DB ein wirkungsloser No-op.
--    Fuer eine echte inkrementelle Migration gegen eine befuellte
--    Produktions-DB waere stattdessen diese Reihenfolge korrekt:
--
--      ALTER TABLE "consultation_sessions" ADD COLUMN "questionnaire_version_id" UUID;
--      UPDATE "consultation_sessions" cs
--        SET "questionnaire_version_id" = (
--          SELECT qv."id" FROM "questionnaire_versions" qv
--          WHERE qv."tenant_id" = cs."tenant_id" AND qv."status" = 'ACTIVE'
--            AND qv."valid_from" <= cs."started_at"
--            AND (qv."valid_to" IS NULL OR qv."valid_to" > cs."started_at")
--          ORDER BY qv."valid_from" DESC LIMIT 1
--        )
--        WHERE cs."questionnaire_version_id" IS NULL;
--      ALTER TABLE "consultation_sessions" ALTER COLUMN "questionnaire_version_id" SET NOT NULL;
-- -----------------------------------------------------------------------

CREATE OR REPLACE FUNCTION forbid_questionnaire_version_change() RETURNS trigger AS $$
BEGIN
  IF NEW.questionnaire_version_id IS DISTINCT FROM OLD.questionnaire_version_id THEN
    RAISE EXCEPTION 'consultation_sessions: questionnaire_version_id ist nach dem Start unveraenderlich (Session %)', OLD.id;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER consultation_sessions_questionnaire_version_immutable
BEFORE UPDATE ON "consultation_sessions"
FOR EACH ROW EXECUTE FUNCTION forbid_questionnaire_version_change();

-- -----------------------------------------------------------------------
-- 6) CustomerAnswer: Append-only-Antwort-Historie statt Ueberschreiben.
--    Jede Aenderung einer Antwort fuegt eine neue Zeile ein; genau eine Zeile
--    pro (tenant, session, question_version) darf gleichzeitig is_active =
--    true haben. Der Service-Layer aendert eine Antwort per Compare-And-Swap
--    in einer Transaktion (UPDATE ... SET is_active = false WHERE ... AND
--    answer_version = $erwartet, danach INSERT der neuen Zeile mit
--    answer_version = $erwartet + 1; 0 betroffene Zeilen -> StaleAnswerVersionError).
--    Bewusst KEIN forbid_update_delete()-Trigger auf dieser Tabelle: der
--    CAS-Flip von is_active ist ein kontrolliertes, beabsichtigtes UPDATE,
--    kein Fremdzugriff (siehe docs/DECISION_LOG.md fuer die Begruendung
--    dieser Abweichung vom sonstigen Append-only-Muster).
-- -----------------------------------------------------------------------

CREATE UNIQUE INDEX "customer_answers_one_active_per_question" ON "customer_answers"
  ("tenant_id", "consultation_session_id", "question_version_id")
