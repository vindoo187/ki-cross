...
Seit Phase 13 (AP0–AP8, ChatGPT-Abnahme 2026-08-30) ist das Campaign-Feature vollstaendig implementiert und gehaertet, nicht nur das Datenmodell: Admin-Service/-API (`src/server/admin/campaign-admin.ts`, `/api/admin/campaigns`, Draft→Validate→Publish-Workflow analog Question/RuleSet/CommissionModel), Admin-UI (`/admin/campaigns`), eigene RBAC-Permission-Keys (`config.campaigns.view`/`edit`/`publish`, additiv zum bestehenden Katalog aus Phase 8/9/10), ein neuer `CAMPAIGN_ACTIVE`-Bedingungstyp in `PrioritizationRule`/`CrossSellingRule` (Phase 13 AP4) sowie eine Attributionskette von getroffener Kampagnenbedingung zu Empfehlung (`RecommendationCampaignSignal`, Phase 13 AP7) – Details zur Auswertungsmechanik und zum dabei behobenen Fingerprint-Defekt siehe [RECOMMENDATION_ENGINE.md](RECOMMENDATION_ENGINE.md) Abschnitt "Campaign-Priorisierung und Attribution". Security/Tenant-Isolation/Parallelitaet und eine vollstaendige Playwright-E2E-Suite (Desktop+Tablet) sind in Phase 13 AP8 verifiziert. Bewusst zurueckgestellt: Cross-Selling-Attribution (`CrossSellingRule` kann `CAMPAIGN_ACTIVE` nutzen, aber `RecommendationCampaignSignal` hat aktuell nur eine FK auf `RecommendationItem`, nicht auf Cross-Selling-Signale – dokumentierte Luecke, siehe [DECISION_LOG.md](DECISION_LOG.md)) sowie ein dediziertes Kampagnen-KPI-Dashboard.
