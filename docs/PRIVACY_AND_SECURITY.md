...
## Umsetzung in der Fragen-Engine (Phase 3A)

Die in Phase 3A implementierte Fragen-Engine (`src/server/questionnaire/`,
siehe [QUESTION_ENGINE.md](QUESTION_ENGINE.md), Abschnitt
"Datenschutzgrenzen") hält sich an die oben beschriebenen Grundsätze:
`CustomerAnswer` speichert ausschließlich strukturierte Antwortwerte ohne
Klarnamen-Zwang; die zu Fragebogen-Ereignissen erzeugten
`AnalyticsEvent`/`AuditLog`-Einträge (`QUESTIONNAIRE_STARTED`,
`QUESTIONNAIRE_COMPLETED`) enthalten ausschließlich IDs und Zählwerte,
niemals Antwortinhalte – verifiziert in
`tests/integration/questionnaire-engine.test.ts`, Fall 36. Jeder Zugriff
läuft über den mandantengescopten `db`-Client (siehe "Rollen- und
Rechtekonzept" unten sowie `DECISION_LOG.md`).
