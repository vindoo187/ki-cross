/**
 * `POST /api/admin/rule-sets/[id]/versions/[versionId]/cross-selling-rules`
 * (Phase 9 AP3, siehe PHASE_9_IMPLEMENTATION_PLAN.md Abschnitt 5). Fuegt
 * eine `CrossSellingRule` inkl. Conditions zu einer DRAFT-Version hinzu.
 *
...
import { createCrossSellingRuleSchema } from "@/server/admin/rule-schemas";
import { addCrossSellingRuleToDraft } from "@/server/admin/rule-admin";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      ...
      const parsed = createCrossSellingRuleSchema.safeParse(body);
      if (!parsed.success) {
      ...
      const rule = await addCrossSellingRuleToDraft(id, versionId, parsed.data);
