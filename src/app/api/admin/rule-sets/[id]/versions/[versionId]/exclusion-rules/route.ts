/**
 * `POST /api/admin/rule-sets/[id]/versions/[versionId]/exclusion-rules`
 * (Phase 9 AP3, siehe PHASE_9_IMPLEMENTATION_PLAN.md Abschnitt 5). Fuegt
 * eine `ExclusionRule` inkl. Conditions zu einer DRAFT-Version hinzu.
 *
...
import { createExclusionRuleSchema } from "@/server/admin/rule-schemas";
import { addExclusionRuleToDraft } from "@/server/admin/rule-admin";
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      ...
      const parsed = createExclusionRuleSchema.safeParse(body);
      if (!parsed.success) {
      ...
      const rule = await addExclusionRuleToDraft(id, versionId, parsed.data);
