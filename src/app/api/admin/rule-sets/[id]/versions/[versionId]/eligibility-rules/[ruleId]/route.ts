/**
 * `PATCH`/`DELETE`
 * `/api/admin/rule-sets/[id]/versions/[versionId]/eligibility-rules/[ruleId]`
 * (Phase 9 AP3, siehe PHASE_9_IMPLEMENTATION_PLAN.md Abschnitt 5). Bearbeitet
 * bzw. entfernt eine `EligibilityRule` in einer DRAFT-Version. Serverseitige
...
import { updateEligibilityRuleSchema } from "@/server/admin/rule-schemas";
import {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      ...
      const parsed = updateEligibilityRuleSchema.safeParse(body);
      if (!parsed.success) {
      ...
      const rule = await updateEligibilityRuleInDraft(id, versionId, ruleId, parsed.data);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      requireConfigPermission(session, "config.rules.edit");
      const { id, versionId, ruleId } = await params;
      await removeEligibilityRuleFromDraft(id, versionId, ruleId);
