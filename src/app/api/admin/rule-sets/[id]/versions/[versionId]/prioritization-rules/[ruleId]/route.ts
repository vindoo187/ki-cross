/**
 * `PATCH`/`DELETE`
 * `/api/admin/rule-sets/[id]/versions/[versionId]/prioritization-rules/[ruleId]`
 * (Phase 9 AP3, siehe PHASE_9_IMPLEMENTATION_PLAN.md Abschnitt 5). Bearbeitet
 * bzw. entfernt eine `PrioritizationRule` in einer DRAFT-Version.
 *
...
import { updatePrioritizationRuleSchema } from "@/server/admin/rule-schemas";
import {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      ...
      const parsed = updatePrioritizationRuleSchema.safeParse(body);
      if (!parsed.success) {
      ...
      const rule = await updatePrioritizationRuleInDraft(id, versionId, ruleId, parsed.data);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      requireConfigPermission(session, "config.rules.edit");
      const { id, versionId, ruleId } = await params;
      await removePrioritizationRuleFromDraft(id, versionId, ruleId);
