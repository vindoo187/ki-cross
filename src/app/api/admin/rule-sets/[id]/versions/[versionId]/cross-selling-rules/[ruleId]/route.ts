/**
 * `PATCH`/`DELETE`
 * `/api/admin/rule-sets/[id]/versions/[versionId]/cross-selling-rules/[ruleId]`
 * (Phase 9 AP3, siehe PHASE_9_IMPLEMENTATION_PLAN.md Abschnitt 5). Bearbeitet
 * bzw. entfernt eine `CrossSellingRule` in einer DRAFT-Version.
 *
...
import { updateCrossSellingRuleSchema } from "@/server/admin/rule-schemas";
import {
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      ...
      const parsed = updateCrossSellingRuleSchema.safeParse(body);
      if (!parsed.success) {
      ...
      const rule = await updateCrossSellingRuleInDraft(id, versionId, ruleId, parsed.data);
...
export async function DELETE(request: NextRequest, { params }: RouteParams) {
  return withErrorMapping(() =>
    withRequestTenantContext(request, async (session) => {
      requireConfigPermission(session, "config.rules.edit");
      const { id, versionId, ruleId } = await params;
      await removeCrossSellingRuleFromDraft(id, versionId, ruleId);
